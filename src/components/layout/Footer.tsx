...
import { Instagram, Twitter, Music, Mail, MapPin, Phone, ArrowRight, Loader2, CheckCircle2 } from "lucide-react";
...
export function Footer() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus('success');
        setMessage(data.message || "¡Bienvenido a la familia!");
        setEmail("");
      } else {
        setStatus('error');
            ...
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div className="relative">
                ...
                <button 
                  type="submit" 
                  disabled={status === 'loading' || status === 'success'}
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-12 h-12 bg-brand-primary rounded-full flex items-center justify-center text-brand-black hover:bg-white transition-all duration-500 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {status === 'loading' ? (
              ...
              {status !== 'idle' && (
                <p className={`text-[10px] font-black uppercase tracking-widest mt-4 px-4 ${
                  status === 'success' ? 'text-green-500' : 'text-brand-secondary'
                }`}>
                  {message}
                </p>
              )}
